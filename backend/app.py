...
def ai_chat():
    try:
        ...
        is_distress = any(word in message.lower() for word in distress_words)
        ...
        return jsonify({
            "success": True,
            "reply": "Main yahaan hoon tumhare liye 💜 Kya tum theek ho?",
            "isDistress": False,
            "showSOS": False
        })
...
def start_trip():
    try:
        data = request.get_json()
        user_name = data.get('userName', 'SafeHer User')
        destination = data.get('destination', '')
        duration_mins = data.get('durationMins', 30)
        contacts = data.get('contacts', [])
        location = data.get('location', {})
        ...
        tracking_link = f"https://safe-her.xyz/track.html?trip={trip_id}"
        for contact in contacts:
            phone = contact.get('phone', '')
            name = contact.get('name', '')
            if phone:
                try:
                    twilio_client.messages.create(
                        body=f"SafeHer: {user_name} has started a trip to {destination}. Track live: {tracking_link} - They will arrive in {duration_mins} mins.",
                        from_=TWILIO_PHONE_NUMBER,
                        to=phone
                    )
        ...
        return jsonify({
            ...
            "message": f"Trip started! Contacts notified."
        })
    ...
    try:
        ...
        for trip in db['trips']:
            if trip['id'] == trip_id:
                ...
                trip['lastUpdate'] = datetime.now().isoformat()
                break
...
def end_trip():
    try:
        data = request.get_json()
        trip_id = data.get('tripId')
        arrived_safe = data.get('arrivedSafe', True)
        ...
        for trip in db['trips']:
            if trip['id'] == trip_id:
                trip['status'] = 'COMPLETED' if arrived_safe else 'SOS'
                trip['arrived'] = arrived_safe
                trip['endTime'] = datetime.now().isoformat()

                # Notify contacts
                for contact in trip.get('contacts', []):
                    phone = contact.get('phone', '')
                    if phone:
                        try:
                            msg = f"SafeHer: {trip['userName']} has arrived safely at {trip['destination']} ✅" if arrived_safe else f"SafeHer ALERT: {trip['userName']} may need help! Last location shared."
                            twilio_client.messages.create(
                                body=msg,
                                from_=TWILIO_PHONE_NUMBER,
                                to=phone
